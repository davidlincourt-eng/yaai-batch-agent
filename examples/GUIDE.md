...
All examples use **Anthropic Claude** as the LLM provider.
...
Use transaction **`STVARV`** (the dedicated TVARVC maintenance transaction) → **New Entries** → fill in Name, Type `P`, value in the **Low** field → **Save**.
...
| `YAAI_ANTHROPIC_API_KEY` | `sk-ant-...` (**never hardcode in programs**) |

> Alternatively, `ycl_aai_api_key_env` reads the key from an OS environment variable — preferred for developer editions running locally.

### 4. SSL certificate for Anthropic (transaction STRUST)
Import Anthropic's root CA cert into the **SSL client PSE** (ANONYM or DEFAULT).
See [abapGit SSL guide](https://docs.abapgit.org/other/ssl-setup.html) for detailed steps.
...
**Run:** SE38, create program `ZYAAI_EX1_SIMPLE_CHAT`, paste code, F8.

Key calls:
```abap
DATA(lo_conn)   = NEW ycl_aai_conn( i_api = yif_aai_const=>c_anthropic ).
DATA(lo_claude) = NEW ycl_aai_anthropic( i_model = '...' i_o_connection = lo_conn ).
lo_claude->chat( EXPORTING i_message = '...' IMPORTING e_t_response = DATA(lt_response) ).
```
...
Key calls:
```abap
lo_claude->set_system_instructions( lv_sys ).   " called BEFORE first chat()
```

---

### Example 4 — Function Calling (`ex4a/b/c`)
**What it does:** Claude automatically calls ABAP methods (`add`, `multiply`) when it needs to compute numbers.
...
| `ycl_aai_anthropic` | Claude chat client |
| `ycl_aai_func_call_anthropic` | Tool/function-calling registry |
| `yif_aai_const` | Constants (e.g. `c_anthropic`) |
| `ycl_aai_api_key_env` | Reads API key from OS env var |
...
| `"Subpath 'chat/completions' is not allowed"` | Anthropic model deployed, but yaai sends OpenAI format | Anthropic models on AI Core use a different API path; use a GPT deployment (`gpt-4.1`, `gpt-4o`) with `ycl_aai_openai` |
