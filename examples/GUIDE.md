...
| Cockpit chatbot sends no response | `YCL_AAI_ASYNC_CHAT_OPENAI` uses standard `ycl_aai_conn` which has no OAuth2 token and no `AI-Resource-Group` header | Replace `NEW ycl_aai_conn( c_openai )` with `NEW zcl_yaai_aicore_conn( )->get_connection( )` and add `use_completions( abap_true )` in `YCL_AAI_ASYNC_CHAT_OPENAI` |
